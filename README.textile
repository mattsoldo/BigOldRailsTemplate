...
bq. This template has not been tested with the new architecture of the Rails 3.0 master branch. It's not going to run with Rails 3 without substantial work.
...
# Clone BigOldRailsTemplate to somewhere on your hard drive (let's say @/templates/BigOldRailsTemplate@ though of course you can put it anywhere).
# Copy @/templates/BigOldRailsTemplate/configs/lark/config.yml@ to @~/.big_old_rails_template/configs/lark/config.yml@ - this gives you a copy of the config file that will override the defaults and not get replaced if you update the template.
# Edit the file you just copied to control the behavior of the template; it's pretty well commented.
# @rails NewProjectName -m /rails/BigOldRailsTemplate/templates/lark/main.rb@ will then use your installed copy of Rails to create NewProjectName based on the template.

h2. Configuration

You can configure this template to control the application that it generates. See @configs/default/config.yml@ to set persistent configuration information. The template will use a configuration file located at @~/.big_old_rails_template/configs/lark/config.yml@ if it finds one; otherwise, it will use the copy embedded in the template's @configs/default@ directory. If you don't have a @config.yml@, or it doesn't have an option that the template is looking for, you'll be prompted at runtime for some of the information, and reasonably sensible defaults will be used for the rest. You should review config.yml before you first run the template.
...
* Uses Erubis and the "rails_xss":http://github.com/NZKoz/rails_xss plugin to provide safe HTML
...
* "script/migrator":http://mediumexposure.com/scriptmigrator-for-navigating-migrations/ to quickly move the database to a specified migration.
...
* "cap_gun":http://github.com/relevance/cap_gun to send emails on deployment.
...
* "rcov_plugin":http://github.com/commondream/rcov_plugin/ to easily run tests with rcov.
...
* rake doc:dnote creates an HTML page of tasks in the doc folder. Uses "d'note":http://proutils.github.com/dnote/.
